 ...
 * 而是使用函数实现的真实代码替换每一次的函数调用(实际是拷贝过去)。
 *
 * 注意：
 * 1，inline 关键字只能提高带有 lambda 参数的函数的性能。因为非inline函数，在使用lambda的时候，编译器会生成匿名类处理。
 * 2，你还是应该注意代码的长度。如果内联的函数很大，将它的字节码拷贝到每一个调用点将会极大地增加字节码的长度。在这种情况下，
 *    你应该将那些与 lambda 参数无关的代码抽取到一个独立的非内联函数中。在 Kotlin 标准库中的内联函数总是很小的 。
 *
