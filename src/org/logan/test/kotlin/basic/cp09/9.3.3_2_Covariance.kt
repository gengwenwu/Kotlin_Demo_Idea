...
// 因为它既含有接收类型为 T 的值作为参数的方法，也含有返回这种值的方法（T同时出现在 in 和 out 位置上）
// 编译器强制实施了这种限制。
...
// 4，构造方法的参数既不在 in位置，也不在 out位置。
// 即使类型参数声明成了 out，仍然可以在构造方法参数的声明中使用它，如下：
class Herd<out T : Animal>(vararg animals: T)


// 5，如果在构造方法的参数上使用了关键字 val 和 var，就会自动声明一个getter和一个setter (如果属性是可变的)。
...
class Herd3<out T : Animal>(// 支持协变
    private var leadAnimal: T, // leadAnimal 的 T 不在in和out位置，因为它是private的，所有setter也不会有问题。
    vararg animals: T
)
