...
/**
 * desc: Kotlin扩展函数：为某种类型添加新的 api 函数。<br/>
 * 扩展函数实质是静态函数，该性质也决定了它不能被子类重写。<br/>
 * 在扩展函数中，可以直接访问被扩展的类的其它方法和属性，就好像是在这个类自己的方法中访问它们一样。 <br/>
 * -
 * 注意，扩展函数并不允许你打破 Java 的封装性。与类内部定义的方法不同的是，扩展函数不能访问私有或受保护的成员。 <br/>
 * 在扩展函数的内部，可以在接收者上调用该类的任意方法，即：无论成员函数还是扩展函数 (不能是 private、protected )。 <br/>
